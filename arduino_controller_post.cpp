...
void connectToWiFi() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        Serial.print(".");
        retries++;
        if (retries > 40) { // Increased retries to 20 seconds (40 * 500ms) before re-attempting from scratch
            Serial.println("\nFailed to connect to WiFi after multiple retries. Re-initiating connection...");
            // Optionally, you can try to re-initialize WiFi.begin() here if needed,
            // but the loop will naturally keep trying due to the while condition.
...
void sendPostRequest(int buttonId) { // buttonId is now the index + 1 for user-friendly IDs
    ...
    // 96 bytes is allocated for a payload like {"mac": "XX:XX:XX:XX:XX:XX", "buttonId": N}
    StaticJsonDocument<96> doc;
    
    // Add the unique device ID (MAC address) to the JSON payload
    doc["mac"] = deviceMAC;
    doc["buttonId"] = buttonId; // Add the button ID
...
void loop() {
    ...
    for (int i = 0; i < NUM_BUTTONS; i++) {
        ...
        if ((millis() - lastDebounceTime[i]) > DEBOUNCE_DELAY_MS) {
            if (reading != lastButtonState[i]) {
                if (reading == LOW) { // If button is pressed (LOW due to INPUT_PULLUP)
                    Serial.print("Button ");
